...
async def handle_confirm_yes(update: Update, context: ContextTypes.DEFAULT_TYPE, params=None):
    ...
    try:
        ...
        # --- Delete Product Logic ---
             ...
             # CRITICAL: Check if product has active reservations before deleting
             c.execute("SELECT reserved FROM products WHERE id = ?", (product_id,))
             product_check = c.fetchone()
             
             if not product_check:
                 conn.rollback()
                 success_msg = f"❌ Error: Product ID {product_id} not found."
                 next_callback = "adm_manage_products"
             elif product_check['reserved'] > 0:
                 conn.rollback()
                 success_msg = f"⚠️ Cannot delete Product ID {product_id}\n\nThis product is currently RESERVED by a customer (reserved: {product_check['reserved']}).\n\nPlease wait for the reservation to expire or for the purchase to complete before deleting."
                 # Try to navigate back to the product list
                 ...
                 logger.info(f"Admin Action (confirm_remove_product): Deleting product ID {product_id} (no active reservations)")
...
async def handle_adm_bulk_price_district(update: Update, context: ContextTypes.DEFAULT_TYPE, params=None):
    """Handle district selection for district scope."""
    query = update.callback_query
    if not is_primary_admin(query.from_user.id):
        return await query.answer("Access denied.", show_alert=True)
